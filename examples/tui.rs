...
impl App {
    ...
    fn setting_value(&self, idx: usize) -> String {
        match idx {
            ...
            2 => format!("{} (~{} Hz)", self.sidetone, 4000 / self.sidetone as u16),
            3 => if self.sidetone_on { "ON" } else { "OFF" }.into(),
            4 => {
            ...
            5 => format!("{}", self.ratio),
            6 => self.paddle_mode.label().into(),
            7 => if self.ptt_on { "ON" } else { "OFF" }.into(),
            8 => format!("{} ({}ms)", self.ptt_lead_in, self.ptt_lead_in as u16 * 10),
            9 => format!("{} ({}ms)", self.ptt_tail, self.ptt_tail as u16 * 10),
            10 => match self.hang_time {
                0 => "1.0 wordspace".into(),
                1 => "1.33 wordspace".into(),
                2 => "1.67 wordspace".into(),
                _ => "2.0 wordspace".into(),
            },
            11 => if self.tune { "ON" } else { "OFF" }.into(),
            12 => if self.pause { "ON" } else { "OFF" }.into(),
            _ => String::new(),
        }
    }
}
    ...
    match app.selected {
        ...
        2 => {
            ...
            app.sidetone = match dir {
                Adjustment::Increment => app.sidetone.saturating_add(1).min(10),
                Adjustment::Decrement => app.sidetone.saturating_sub(1).max(1),
            };
